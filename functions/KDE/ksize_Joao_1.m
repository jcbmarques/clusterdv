function npd = ksize_Joao_1(npd,type,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%%
%get dimentions and nymber of points
  Nd = npd.D; Np = npd.N;
  ...
  %%
  %%%%%%%%%%% normalize data if tyoe ends in p%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  type = lower(type);
  if (type(end)=='p')
  ...
  case {'local','localp'},                   % local likelihood cross-val
    if (nargin < 3)%if there is no vector use neighbor distance
      
      %prop is nearest neighbor distances
      [prop,minm,maxm] = neighborDistance(npd,sqrt(getNpts(npd)));
      
    else prop = varargin{1}; [minm,maxm] = neighborMinMax(npd);%use vector with distances
