...
void kasum(const BallTreeDensity& dens, const BallTree& loc, double *e) 
{
  ...
  unsigned int Nd = dens.Ndim();
  double Cnorm = 1.0/(pow(pi,((double)Nd)/2)*pow((double)2,(double)(Nd+1)));
  unsigned int k;
  BallTree::index i,j;
  unsigned long jj;
...
void kukprmconv(const BallTreeDensity& dens, double a, bool erfFlag, double *fr) 
{
  ...
  unsigned int Nd = dens.Ndim();
  unsigned long Np = dens.Npts();
  double aNd  = pow((double)a,(double)Nd);
  BallTree::index j;
  unsigned long jj;
  unsigned int i,k;
...
void entDirISE(const BallTreeDensity& dens, double* err) 
{
  ...
  BallTree::index j;
  unsigned long jj;
  double Cnorm = 1.0/(pow((double)pi,((double)Nd)/(double)2)*pow((double)2,(double)(Nd+1)));
